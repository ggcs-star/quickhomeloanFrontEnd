...
/* ---------------------------------------------
   MAIN COMPONENT
--------------------------------------------- */
export default function LoanVsFdCalculator() {
  /* ---------------- INPUT STATE ---------------- */
  const [principal, setPrincipal] = useState(50000);
  const [loanInterest, setLoanInterest] = useState(5);
  const [fdInterest, setFdInterest] = useState(20);
  const [inflation, setInflation] = useState(4);
  const [term, setTerm] = useState(2);
  const [assetGrowth, setAssetGrowth] = useState(0);
  ...
  const handleCalculate = () => {
    ...
    const realFdValue =
      fdMaturity / Math.pow(1 + inflation / 100, term);

    const assetFutureValue =
      principal * Math.pow(1 + assetGrowth / 100, term);
    ...
    setResults({
      ...
      recommendation:
        netAdvantage > 0
          ? "Buying the asset using a loan is financially better"
          : "Investing in FD is financially better",
    });
  ...
  const progress =
    hasCalculated && results
      ? principal / results.assetFutureValue
      : 0;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3">
          ...
          <div className="space-y-6 pt-4 border-t border-gray-200">
            ...
            <InputField label="Inflation Rate (% p.a.)" value={inflation} onChange={setInflation} />
            <InputField label="Term (Years)" value={term} onChange={setTerm} />
            <InputField label="Asset Growth Rate (% p.a.)" value={assetGrowth} onChange={setAssetGrowth} />
          </div>
        ...
        <div className="lg:col-span-2 bg-gray-100 rounded-xl p-6 flex flex-col items-center justify-center">
          ...
          {/* DONUT CONTAINER (USED FOR BOTH STATES) */}
          <div className="relative w-[180px] h-[180px] mb-6">
            ...
            {/* CENTERED TEXT – SAME FOR BOTH STATES */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
              {!hasCalculated ? (
                <span className="text-gray-500 font-medium">
                  Calculate Loan V/S EMI
                </span>
              ) : (
                <>
          ...
          {!hasCalculated ? (
            ...
            <>
              ...
              <div className="w-full space-y-3">
                ...
                <Stat label="FD Value (After Inflation)" value={formatCurrency(results.realFdValue)} />
                <Stat label="Asset Future Value" value={formatCurrency(results.assetFutureValue)} />
                <Stat
                  label="Net Advantage (Real)"
                  value={
