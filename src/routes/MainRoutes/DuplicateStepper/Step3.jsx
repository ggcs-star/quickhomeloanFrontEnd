...
export default function Step3({ formData, setStep, token }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/loan/submit-form`, payload, {
        headers: { 
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="p-4 bg-brand-50 rounded-lg border border-brand-100">
                ...
                <ul className="text-sm space-y-1 text-slate-700">
                  <li className="flex justify-between">
                    <span>PAN:</span>
                    <span className="font-medium font-mono">{formData.pan || "Not provided"}</span>
                  </li>
                  <li className="flex justify-between">
                    <span>Name:</span>
                  ...
                  <li className="flex justify-between">
                    ...
                    <span className="font-medium">₹{formData.income?.toLocaleString()}</span>
                  </li>
                  <li className="flex justify-between">
                    <span>Loan Required:</span>
                    <span className="font-medium">₹{formData.loan?.toLocaleString()}</span>
                  </li>
                  <li className="flex justify-between">
                    <span>City:</span>
                  ...
                  <li className="flex justify-between">
                    ...
                    <span className="font-medium">{propertyStage}</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="flex gap-3">
                <button 
                  ...
                  className="flex-1 px-4 py-3 rounded-lg border-2 border-slate-200 text-slate-600 hover:border-brand-500 disabled:opacity-50 transition-colors"
                >
                  Back
                </button>
                <button 
